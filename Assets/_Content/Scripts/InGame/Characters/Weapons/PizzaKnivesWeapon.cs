...
{
	public class PizzaKnivesWeapon: WeaponBase
	{
		[SerializeReference] [Expandable] protected PizzaKnivesWeaponSettings _settings;
		[SerializeField] private PizzaKnife _pizzaKnifePrefab;
		...
		{
			var count = _settings.GetKnivesCount(_weaponLevel);
			...
			for (int i = 0; i < count; i++)
			{
				...
				var pizzaKnife = Instantiate(_pizzaKnifePrefab, _character.transform.position, Quaternion.identity);
				pizzaKnife.Shoot(direction, damage);
			}
		}
	}
}