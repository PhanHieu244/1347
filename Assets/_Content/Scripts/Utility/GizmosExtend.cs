...
namespace Kit
{
	...
	public static class GizmosExtend
	{
		...
		public static void DrawCircle(Vector3 position, Vector3 up = default(Vector3), Color color = default(Color),
			float radius = 1.0f)
		{
		...
		/// <param name='color'>- The color of the circle.</param>
		/// <param name='radius'>- The radius of the circle.</param>
		public static void DrawCircleWithHandles(Vector3 position, Vector3 up = default(Vector3),
			Color color = default(Color), float radius = 1.0f)
		{
#if UNITY_EDITOR_64
		...
		public static void DrawCylinderWithHandles(Vector3 start, Vector3 end, Color color = default(Color),
			float radius = 1.0f)
		{
#if UNITY_EDITOR_64
			if (IsHandleHackAvailable)
			{
				...
				//Radial circles
				DrawCircleWithHandles(start, up, color, radius);
				DrawCircleWithHandles(end, -up, color, radius);
				DrawCircleWithHandles((start + end) * 0.5f, up, color, radius);
				...
				using (new HandleColorScope(color))
				{
					...
					//Start endcap
					Handles.DrawLine(start - right, start + right);
					Handles.DrawLine(start - forward, start + forward);

					//End endcap
					Handles.DrawLine(end - right, end + right);
					Handles.DrawLine(end - forward, end + forward);
				}
			}
#endif
		}
		...
		public static void DrawArrow(Vector3 position, Vector3 direction, Color color = default(Color),
			float angle = 15f, float headLength = 0.3f)
		{
		...
		public static void DrawCapsule(Vector3 point1, Vector3 point2, float radius = 1f, Color color = default(Color))
		{
			...
			else
			{
				...
				using (new ColorScope(color))
				{
					...
					{
						//Start endcap
						Gizmos.DrawLine(Vector3.Slerp(right, -up, i / 25.0f) + point1,
							Vector3.Slerp(right, -up, (i - 1) / 25.0f) + point1);
						Gizmos.DrawLine(Vector3.Slerp(-right, -up, i / 25.0f) + point1,
							Vector3.Slerp(-right, -up, (i - 1) / 25.0f) + point1);
						Gizmos.DrawLine(Vector3.Slerp(forward, -up, i / 25.0f) + point1,
							Vector3.Slerp(forward, -up, (i - 1) / 25.0f) + point1);
						Gizmos.DrawLine(Vector3.Slerp(-forward, -up, i / 25.0f) + point1,
							Vector3.Slerp(-forward, -up, (i - 1) / 25.0f) + point1);

						//End endcap
						Gizmos.DrawLine(Vector3.Slerp(right, up, i / 25.0f) + point2,
							Vector3.Slerp(right, up, (i - 1) / 25.0f) + point2);
						Gizmos.DrawLine(Vector3.Slerp(-right, up, i / 25.0f) + point2,
							Vector3.Slerp(-right, up, (i - 1) / 25.0f) + point2);
						Gizmos.DrawLine(Vector3.Slerp(forward, up, i / 25.0f) + point2,
							Vector3.Slerp(forward, up, (i - 1) / 25.0f) + point2);
						Gizmos.DrawLine(Vector3.Slerp(-forward, up, i / 25.0f) + point2,
							Vector3.Slerp(-forward, up, (i - 1) / 25.0f) + point2);
					}
				}
		...
		public static void DrawPlane(Vector3 start, Vector3 end, Vector3 upward, float height = 1f,
		...
		/// <param name="color"></param>
		public static void DrawDirection(Vector3 position, Vector3 direction, float distance = 1f,
			Color color = default(Color))
		{
			using (new ColorScope(color))
			{
		...
		public static void DrawLabel(Vector3 position, string text, GUIStyle style = default(GUIStyle),
			Color color = default(Color), float offsetX = 0f, float offsetY = 0f)
		{
#if UNITY_EDITOR
			if (IsHandleHackAvailable)
			{
		...
		[System.Diagnostics.Conditional("UNITY_EDITOR")]
		public static void DrawAngleBetween(Vector3 center, Vector3 from, Vector3 to, Vector3 axis, float radius,
			Color color, bool constantScreenSize = true, bool label = false)
		{
#if UNITY_EDITOR
			if (IsHandleHackAvailable)
			{
		...
		public static void DrawBoxCastOnHit(Vector3 origin, Vector3 halfExtents, Quaternion orientation,
			Vector3 direction, float hitInfoDistance, Color color = default(Color))
		{
		...
		public static void DrawBoxCastBox(Vector3 origin, Vector3 halfExtents, Quaternion orientation,
			Vector3 direction, float distance, Color color = default(Color))
		{
			direction.Normalize();
		...
		private static void DrawLocalCube(ref Color color, ref Vector3 lbb, ref Vector3 rbb, ref Vector3 lbf,
			ref Vector3 rbf, ref Vector3 lub, ref Vector3 rub, ref Vector3 luf, ref Vector3 ruf)
		{
			using (new ColorScope(color))
			{
		...
		public static void DrawLocalCube(Transform transform, Vector3 size, Color color = default(Color),
		...
		/// <param name='center'>- The local position of the cube.</param>
		/// <param name='color'>- The color of the cube.</param>
		public static void DrawLocalCube(Matrix4x4 space, Vector3 size = default(Vector3), Color color = default(Color),
			Vector3 center = default(Vector3))
		{
		...
		/// <param name='size'>- The local size of the cube.</param>
		/// <param name='color'>- The color of the cube.</param>
		public static void DrawLocalCube(Vector3 position, Quaternion rotation, Vector3 size = default(Vector3),
			Color color = default(Color))
		{
