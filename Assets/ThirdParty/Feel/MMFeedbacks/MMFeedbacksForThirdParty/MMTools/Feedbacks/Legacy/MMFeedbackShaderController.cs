...
namespace MoreMountains.Feedbacks
{
	...
	public class MMFeedbackShaderController : MMFeedback
	{
		...
		[Header("Float Controller")] public bool OnlyEnabledTargets;
		...
		{
			if (!Active || !FeedbackTypeAuthorized || (TargetShaderController == null && TargetShaderControllerList.Count == 0))
			{
				return;
			}
			...
			if (TargetShaderController != null && (!OnlyEnabledTargets || OnlyEnabledTargets && TargetShaderController.gameObject.activeSelf))
			...
			{
				if (OnlyEnabledTargets && !shaderController.gameObject.activeSelf)
					continue;
